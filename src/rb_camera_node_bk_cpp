...
static GstFlowReturn processData(GstElement * sink, RbCamera::CustomData * data){
  ...
  if(sample){
    ...
    cv::Mat frame_rgb = cv::Mat::zeros(480, 640, CV_8UC3);
    cv::Mat frame(cv::Size(640, 480), CV_8UC4, (char*)map_info.data, cv::Mat::AUTO_STEP);
    cv::cvtColor(frame, frame_rgb, cv::COLOR_RGBA2BGR);
...
RbCamera::RbCamera(){
  ...
  // Build pipeline
  gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.convert, data.filter, data.appsink, nullptr);
  ...
  //g_object_set (G_OBJECT(data.source), "camera", 0, nullptr);
