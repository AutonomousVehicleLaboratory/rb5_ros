...
static GstFlowReturn processData(GstElement * sink, RbCamera::CustomData * data){
  ...
  if(sample){
    ...
    cv::Mat frame(cv::Size(width, height*3/2), CV_8UC1, (char*)map_info.data);
    cv::cvtColor(frame, frame_rgb, cv::COLOR_YUV2RGBA_NV12);
...
RbCamera::RbCamera(){
  ...
  if (gst_element_link_many (data.source, data.capsfilter, data.filter, data.appsink, nullptr) != TRUE) {
